...
- Built-in CSS styles: The package now includes pre-built CSS (`dist/styles.css`) that contains all required Tailwind utilities
- CSS export in package.json for easy importing: `import "nextjs-tour/styles.css"`
- Standalone CSS build process using Tailwind CSS and PostCSS
- Automatic style extraction during build to ensure all component styles are available

### Changed

- Consumers no longer need to configure their Tailwind config to scan library files
- Simplified setup: just import the CSS file and use the components
- Updated documentation with CSS import instructions

### Fixed

- Tour overlay visibility issues when Tailwind utilities were purged
- Removed dependency on consumer's Tailwind configuration for library styles
